...
import { Box, Button, Stack, Typography, Chip, Tooltip, Badge, Divider, Paper } from "@mui/material";
...
const DetailItemDescription = (props) => {
        ...
        .then(function (response) {
          ...
          specs.forEach(spec => {
            if (spec.specificationName === "Kích thước" && spec.specificationValue) {
              const sizeValues = spec.specificationValue.split(",").map(s => s.trim());
              sizes.push(...sizeValues);
            }
            if (spec.specificationName === "Màu sắc" && spec.specificationValue) {
              const colorValues = spec.specificationValue.split(",").map(c => c.trim());
              colors.push(...colorValues);
            }
            if (spec.specificationName === "Số lượng" && spec.specificationValue) {
              stock = parseInt(spec.specificationValue.trim()) || 0;
            }
          });
          ...
          setProductStock(stock > 0 ? stock : 100);
  ...
  if (loading) {
    return (
      <Box className="loading-container" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px' }}>
        <Typography variant="body1" sx={{ color: '#666' }}>Đang tải thông tin sản phẩm...</Typography>
      </Box>
    );
  }
  ...
  if (!dataDes && !loading) {
    return (
      <Box className="error-container" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px' }}>
        <Typography variant="body1" sx={{ color: '#e63946' }}>Không thể tải thông tin sản phẩm. Vui lòng thử lại sau.</Typography>
      </Box>
    );
  }
    ...
    <div className="ContainerDes">
      <Paper elevation={0} sx={{ p: 3, borderRadius: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
          ...
          />
          <Chip 
            label={`Loại: ${dataDes.category?.categoryName || "Áo thun có trơn tay ngắn"}`} 
            size="small"
            color="secondary"
            variant="outlined"
          />
        ...
        {/* Tên sản phẩm */}
        <Typography variant="h5" component="h1" sx={{ fontWeight: 'bold', mb: 2, color: '#1A374D' }}>
        ...
        <Box sx={{ 
          ...
          <Typography variant="h4" sx={{ color: '#e63946', fontWeight: 'bold' }}>
            {discountedPrice > 0 
          ...
          {(dataDes.sale || discountedPrice === 0) && (
            <>
              ...
              >
                {dataDes.price > 0 
                  ? dataDes.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") 
                  : "100.000"} đ
              </Typography>
          ...
          <Box sx={{ 
            ...
            <Typography variant="body2" sx={{ color: '#28a745', fontWeight: 'medium' }}>
              Còn hàng
            </Typography>
          </Box>
        </Box>
        ...
        >
          ...
          <Box sx={{ flexGrow: 1 }}>
            <Typography variant="body2" sx={{ fontWeight: 'bold', color: '#e63946' }}>
        ...
        {/* Màu sắc */}
        <Box sx={{ mb: 3 }}>
          <Typography variant="body1" sx={{ fontWeight: 'bold', mb: 1, color: '#1A374D' }}>
            Màu sắc:
          </Typography>
        ...
        {/* Ưu đãi khi mua hàng */}
        <Box sx={{ mb: 3, bgcolor: '#f9f9f9', p: 2, borderRadius: 2, border: '1px solid #eee' }}>
          <Typography variant="body1" sx={{ fontWeight: 'bold', mb: 1, color: '#e63946' }}>
            ƯU ĐÃI KHI MUA HÀNG:
          </Typography>
          <Stack spacing={1.5}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <LocalShippingIcon sx={{ color: '#e63946' }} />
              <Typography variant="body2">Miễn phí ship toàn quốc đơn hàng từ 300.000đ</Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <SyncIcon sx={{ color: '#e63946' }} />
              <Typography variant="body2">Đổi trả miễn phí trong 30 ngày nếu sản phẩm lỗi</Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <SellIcon sx={{ color: '#e63946' }} />
              <Typography variant="body2">Giảm 10% cho đơn hàng tiếp theo khi đánh giá 5 sao</Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <AssignmentReturnIcon sx={{ color: '#e63946' }} />
              <Typography variant="body2">Tích điểm đổi quà với mỗi đơn hàng thành công</Typography>
            </Box>
          </Stack>
        </Box>
